function karatsuba(x, y)  {
    ...
    if ((x_string.length <= 4) || (y_string.length <= 4)) { // minimum 4 digits to use Karatsuba, otherwise use brute force
      ...
      let half_n = Math.ceil(n / 2); 
  
      let x_left = Number(x_string.slice(0, Math.floor(x_string.length/2))); // split X into two numbers
      let x_right = Number(x_string.slice(Math.floor(x_string.length/2)));
  
      let y_left = Number(y_string.slice(0,  Math.floor(y_string.length/2))); // split Y into two numbers
      let y_right = Number(y_string.slice(Math.floor(y_string.length/2)));
  ...
  let result = karatsuba(123456, 67890);
  console.log(result);
